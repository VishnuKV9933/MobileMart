...
const home = async function (req, res, next) {
  ...
  if (req.session.user) {
    let listCount = await wishlistCartManagement.wishListCount(
...
const product = async (req, res) => {
  ...
  if (req.session.user) {
    userLogged = true;
    listCount = await wishlistCartManagement.wishListCount(
      req.session.user._id
    );
      ...
      userHelpers.getGrandTotal(id).then((response) => {
        userHelpers.getUserAddress(id).then((address) => {
          if (response[0]) {
            ...
            if (user.coopon) {
              let couponTotal =
                grandTotal - (grandTotal * user.coopon.percentage) / 100;
...
const applyCoupon = (req, res) => {
  offerHelper
    .cooponChecking(req.body.coopon, req.session.user._id)
    .then((response) => {
      if (response.status) {
        console.log(response);
        res.json(response);
...
const removeCoupun = (req, res) => {
  offerHelper
    .couponUserRemovel(req.session.user._id)
    .then((data) => {
      console.log("hi");
...
const placeOrder = async (req, res) => {
  ...
  if (user.coopon) {
    ...
    let discountTotalPrice = Math.round(
      (parseInt(totalPrice[0].grandTotal) * cooponPercent) / 100
    );
    let paypalDiscountTotalPrice = Math.round(
      (parseInt(paypaltotalPrice[0].total) * cooponPercent) / 100
    );

    totalPrice = parseInt(totalPrice[0].grandTotal) - discountTotalPrice;
    paypaltotalPrice =
      parseInt(paypaltotalPrice[0].total) - paypalDiscountTotalPrice;

    userHelpers
      .placeOrderCoupon(req.body, products, totalPrice, couponObj)
      .then((orderId) => {
        let order = req.body;

        if (order["payment-method"] == "WALLET") {
          userHelpers
            .walletAmountCheck(req.body.userId, totalPrice)
            .then((response) => {
              if (response.walletAmount) {
                offerHelper
                  .cooponObjectRemovelUser(req.session.user._id)
                  .then((data) => {})
                  .catch((err) => {
                    console.log(err);
                  });
                ...
                userHelpers
                  .walletAmountReduce(req.body.userId, totalPrice)
                  .then((data) => {});
...
const verifyPayment = async (req, res) => {
  ...
  userHelpers
    .verifyRazorPayPayment(details.payment)
    .then(() => {
      userHelpers
        .changeOrderStatusOnline(details.order.receipt)
        .then((data) => {});

      userHelpers
        .getOrderProductQuantity(details.order.receipt)
        .then((data) => {
          data.forEach((element) => {
            userHelpers.updateStockDecrease(element);
          });
      ...
      userHelpers.getuserDetails(req.session.user._id).then((user) => {
        if (user.coopon) {
          offerHelper
            .cooponObjectRemovelUser(user._id)
            .then((data) => {})
            .catch((err) => {
              console.log(err);
            });
...
const payPal = (req, res) => {
  ...
  const create_payment_json = {
    ...
    redirect_urls: {
      return_url:
        "http://mobilemart.ml/success?order=" + order + "&?user=" + user,
