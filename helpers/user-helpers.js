...
module.exports = {
  ...
  userCount:()=>{
    ...
    return new Promise((resolve,reject)=>{
      ...
      db.get().collection(collection.USER_COLLECTION).countDocuments().then((data)=>{
        console.log(data);
        resolve(data)
      })
    ...
    if (userCart) {
        ...
        let prodExist = userCart.products.findIndex((product => product.item ==productId));
                ...
                if (prodExist != -1) {
                  ...
                  db.get().collection(collection.CART_COLLECTION).aggregate([{$match:{
    ...
    return new Promise((resolve,reject) =>{
        ...
        .updateOne({_id:objectId(details.cart)},{
          ...
          let signOfCcount =Math.sign(details.count)
                ...
                }else if(signOfCcount==-1&& data[0].stock==data[0].quantity){
...
getOrderProductQuantity:(orderId)=>{
  return new Promise((resolve,reject)=>{
    db.get().collection(collection.ORDER_COLLECTION).aggregate([{$match:{_id:objectId(orderId)}},{
      $unwind:'$products'
    },
    {
      $project:{
...
updateStockDecrease:({productId,quantity})=>{
  db.get().collection(collection.PRODUCT_COLLLECTION).updateOne({_id:objectId(productId)}, {
    $inc: {"stock": -quantity},
})
},
...
let  orders2= await  db.get().collection(collection.ORDER_COLLECTION).aggregate([{$match:{
  $and:[{userId:objectId(userId)},{status:{$nin:['pending']}}]
}}]).sort({"_id":-1}).toArray()
...
orderReturn:(details)=>{
  return new Promise(async(resolve,reject)=>{
    ...
    let orderObj=await db.get().collection(collection.ORDER_COLLECTION).findOne({_id:objectId(details.orderId)})
 ...
 db.get().collection(collection.ORDER_COLLECTION).updateOne({_id:objectId(details.orderId)},{$set:{
  status:'Returned',
  returnStatus:true
...
getAllReturnOrders:()=>{
  ...
  return new Promise((resolve,reject)=>{
    ...
    db.get().collection(collection.RETURN_COLLECTION).find().toArray().then((orders)=>{
      resolve(orders)
    })
...
getReturnOrder:(orderId)=>{
  ...
  return new Promise((resolve,reject)=>{
    ...
    db.get().collection(collection.RETURN_COLLECTION).findOne({orderId:objectId(orderId)}).then((data)=>{
      resolve(data)
    })
  })
...
changeReturnStatus:(details)=>{
  ...
  console.log(details);
    ...
    return new Promise(async(resolve,reject)=>{
      db.get().collection(collection.RETURN_COLLECTION).deleteOne({orderId:objectId(orderId)})
...
getReturnCounts:()=>{
  ...
  return new Promise((resolve,reject)=>{
    db.get().collection(collection.RETURN_COLLECTION).countDocuments().then((data)=>{
      console.log(data);
      resolve(data)
    })
  })
...
addTowallet:(orderId)=>{
 ...
 walletAmount= parseInt(walletAmount+order.totalAmount)
  ...
  db.get().collection(collection.USER_COLLECTION).updateOne({_id:objectId(order.userId)},{$set:{
...
deleteAddress:(id)=>{
  ...
  return new Promise(async(resolve,reject)=>{
 db.get().collection(collection.ADDRESS_COLLECTION).deleteOne({_id:objectId(id)}).then((data)=>{
...
findAddress:(id)=>{
  ...
  return new Promise(async(resolve,reject)=>{
    db.get().collection(collection.ADDRESS_COLLECTION).findOne({_id:objectId(id)}).then((data)=>{
...
editAddress:(data)=>{
  return new Promise(async(resolve,reject)=>{
    ...
    db.get().collection(collection.ADDRESS_COLLECTION).updateOne({_id:objectId(data.addressId)},{$set:{
...
return new Promise(async(resolve,reject)=>{
  ...
  let userMbile =await db.get().collection(collection.USER_COLLECTION).findOne({$and:[{mobile:details.mobile},{_id:{$ne:objectId(details.userId)}}]})
  let useremail =await db.get().collection(collection.USER_COLLECTION).findOne({$and:[{email:details.email},{_id:{$ne:objectId(details.userId)}}]})  
  ...
  console.log(useremail);
                ...
                if(userMbile){
                  ...
                  db.get().collection(collection.USER_COLLECTION).updateOne({_id:objectId(details.userId)},{$set:{
                   name:details.name,
                   email:details.email,
...
filteredProducts:(id)=>{
 ...
 newArr = brand.map(myFunction)
   ...
   console.log(newArr);
      db.get().collection(collection.PRODUCT_COLLLECTION).aggregate([{$match:{brandId:{$in:newArr}}},{
        $lookup:{
...
getSearchResults:(name)=>{
  ...
  return new Promise((resolve,reject)=>{
    ...
    db.get().collection(collection.PRODUCT_COLLLECTION).find({name:{$regex: new RegExp('^'+name+'.*','i')}}).limit(5).toArray()
    .then((data)=>{
